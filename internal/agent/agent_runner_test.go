...
func TestRuntimeSnapshotsComeFromModules(t *testing.T) {
	...
	for _, want := range []string{"project", "skills", "todo", "memory", "messaging", "coc", "subagent", "task_system", "team", "background", "cron"} {
...
func TestDisabledModulesRemoveToolsSnapshotsAndLifecycle(t *testing.T) {
	...
	for _, disabledTool := range []string{"load_skill", "todo_write", "messaging_normalize", "coc_roll_dice", "task", "task_create", "team_send_message", "background_bash", "schedule_cron"} {
		if hasString(names, disabledTool) {
			t.Fatalf("disabled module tool %q should not be exposed in %v", disabledTool, names)
		}
	}
	for _, disabledModule := range []string{"skills", "todo", "memory", "messaging", "coc", "subagent", "task_system", "team", "background", "cron"} {
