...
默认地址是 `http://127.0.0.1:8787`，可以用 `BEE_AGENT_NODE_EDITOR_ADDR=:8787` 修改。当前 Web UI 从后端 node template registry 加载 Prompt/Skill/Toolset/Memory 节点模板，支持切换、新建、复制 blueprint，新增节点、编辑节点 label、用表单编辑 source/path/prompt/tools、拖动节点保存位置、按 typed ports 连线、Agent prompt 输入自动续位、编辑 JSON、保存和校验。校验 API 返回结构化 runtime payload；右侧校验面板会显示运行命令、policy 过滤后的有效工具列表、policy trace、prompt preview 和 capability diagnostics。
