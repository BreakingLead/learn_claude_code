...
- `InjectRelevantMemories`, `InjectBackgroundNotifications`: optional runtime behaviors.
...
`runAgent(ctx, client, spec, messages)` owns the shared state machine. It filters tools, builds the prompt from the selected tool names, calls the model, runs hooks, executes tools, runs module turn hooks, and returns `agentRunResult`.
...
The runner still depends on `agentRuntime` for shared services: tool handlers, hooks, prompt cache, permissions, compaction, memory, and logging. Modules can contribute prompt blocks, tool definitions, tool handlers, and turn hooks through `Module`, `PromptContributor`, `ToolContributor`, and `TurnContributor`.

`todo_write` is implemented as a module. The todo module owns current todo state, exposes the `todo_write` tool, and injects reminder messages through a turn hook when the main agent goes several tool rounds without updating todos.
